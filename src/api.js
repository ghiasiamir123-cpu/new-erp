...
export const warehouseApi = {
  ...
  // اموال: آمار، تاریخچه و بازرسی
  assetsSummary: () => request("/items/assets-summary/"),
  assetEvents: (sku) => request(`/asset-events/${qs({ sku })}`),
  createAssetEvent: (data) => request("/asset-events/", { method: "POST", body: data }),
  removeAssetEvent: (id) => request(`/asset-events/${id}/`, { method: "DELETE" }),
  inspections: () => request("/asset-inspections/"),
  inspection: (id) => request(`/asset-inspections/${id}/`),
  createInspection: (data) => request("/asset-inspections/", { method: "POST", body: data }),
  updateInspection: (id, data) => request(`/asset-inspections/${id}/`, { method: "PATCH", body: data }),
  closeInspection: (id, data) => request(`/asset-inspections/${id}/close/`, { method: "POST", body: data || {} }),
  removeInspection: (id) => request(`/asset-inspections/${id}/`, { method: "DELETE" }),
  valresaFormula: () => request("/items/valresa-formula/"),
  itemVariants: (id) => request(`/items/${id}/variants/`),
  reviewFamilies: (params) => request(`/stock-review/${qs(params)}`),
  reviewFamily: (key) => request(`/stock-review/family/${qs({ key })}`),
  reviewMark: (data) => request("/stock-review/mark/", { method: "POST", body: data }),
  reviewLinkPreview: (data) => request("/stock-review/link-preview/", { method: "POST", body: data }),
  reviewLink: (data) => request("/stock-review/link/", { method: "POST", body: data }),
