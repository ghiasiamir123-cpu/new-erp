...
عدد هر سطر:
    ستون «شمارش». برگه‌ای که ستون شمارش دارد یعنی شمرده شده، پس سطرِ
    خالی یعنی صفر — نه «نامعلوم». اگر هیچ برگه‌ای از یک فایل ستون شمارش
    نداشته باشد (یعنی آن برند اصلاً شمرده نشده) «موجودی سیستم» ملاک
    می‌شود و برگه‌های بی‌شمارشِ فایل‌های شمرده‌شده کنار گذاشته می‌شوند.
    ...
    def _read_file(self, path):
        ...
        for ws, hr, cols in use:
            ...
            for i, row in enumerate(ws.iter_rows(min_row=hr + 1, values_only=True),
                ...
                if qty is None and source == "شمارش":
                    qty = Decimal(0)   # در فرم شمارش، خالی یعنی چیزی نبود
                shelf = ""
                j = cols.get("shelf")
